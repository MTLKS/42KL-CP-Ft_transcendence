...
export class ChatGateway {
	...
	// Received message from client, add emit it to its respective room
	@SubscribeMessage('msgToServer')
	async handleMessage(@MessageBody() body: any, @ConnectedSocket() client: Socket) {
		const ROOM_ID = await this.chatService.joinRoom(this.server, client, body.channelId, body.message);
	...
	async joinRoom(@MessageBody() body: any, @ConnectedSocket() client: Socket){
		await this.chatService.joinRoom(this.server, client, body.roomId, body.message);
	}
