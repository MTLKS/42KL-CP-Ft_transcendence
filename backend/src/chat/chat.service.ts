...
export class ChatService {
	...
	async message(client: any, server: any, intraName: string, message: string): Promise<any> {
		...
		if (USER_DATA.error !== undefined)
			return;
		const MY_ROOM = await this.channelRepository.findOne({ where: {channelName: USER_DATA.intraName, isRoom: false} });
		...
		const CHANNEL = await this.channelRepository.findOne({ where: {channelName: intraName, ownerIntraName: intraName, isPrivate: true, password: null, isRoom: false} });
		if (CHANNEL === null)
			return server.to(MY_ROOM.channelId).emit("message", { error: "Invalid channel - channel is not found" } );
		...
		if (FRIENDSHIP.length !== 1 || FRIENDSHIP[0].status !== "ACCEPTED")
