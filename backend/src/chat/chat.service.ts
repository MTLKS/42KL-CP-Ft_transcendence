...
export class ChatService {
  ...
  async userConnect(client: any): Promise<any> {
    ...
    if (USER_DATA === undefined) return USER_DATA;
    const DM_ROOM = await this.channelRepository.find({
      where: { channelName: USER_DATA.intraName, isRoom: false },
    });
    if (DM_ROOM.length === 0) {
      const NEW_ROOM = await this.channelRepository.save(
        new Channel(
          USER_DATA.intraName,
          USER_DATA.intraName,
          true,
  ...
  async sendMessage(
    ...
    if (message.length > 1024 || message.length < 1)
      return {
        error: 'Invalid message - message is must be 1-1024 characters only',
      };
    const USER_DATA = await this.userService.getMyUserData(
      client.handshake.headers.authorization,
    );
  ...
  findAllMessages(channelId: string): ChatDTO[] {
    const TARGET_ROOM = this.chatRooms.find(
