...
export class ChatService {
  ...
  async createNewDM(accessToken: string, receiverIntraName): Promise<any> {
    ...
    if (receiverIntraName === undefined)
      return {
        error: 'Invalid receiverIntraName - receiverIntraName is undefined',
      };
    if (receiverIntraName === SENDER.intraName)
      return { error: "Invalid receiverIntraName - you can't DM yourself" };
    const RECEIVER = await this.userService.getUserDataByIntraName(
      receiverIntraName,
    );
    if (RECEIVER === undefined)
      return {
        error: 'Invalid receiverIntraName - receiverIntraName is not found',
      };

    const EXISTING = [
      ...(await this.channelRepository.find({
        where: { roommateIntraName: SENDER.intraName },
      })),
      ...(await this.channelRepository.find({
        where: { roommateIntraName: RECEIVER.intraName },
      })),
    ];
    if (EXISTING.length !== 0) return { error: 'DM already exist' };
