...
export class FriendshipService {
  ...
  async friendshipRoom(
    ...
    const FRIENDSHIP = await this.friendshipRepository.find({
      where: {
        senderIntraName: USER_DATA.intraName,
        receiverIntraName: intraName,
      },
    });
    if (FRIENDSHIP.length === 0) return { error: 'Friendship does not exist' };
    server
      .to(intraName)
      .emit('friendshipRoom', {
        senderIntraName: USER_DATA.intraName,
  ...
  async checkJson(
    ...
    if (receiverIntraName == undefined || status == undefined)
      return {
        error:
          'Invalid body - body must include receiverIntraName(string) and status(stirng)',
      };
    if (senderIntraName == receiverIntraName)
      return {
        error: 'Invalid intraName - no friends so you friend yourself?',
      };
    if (
      ...
      return {
        error: 'Invalid status - status must be PENDING, ACCEPTED or BLOCKED',
      };
    if (
      (
        await this.userRepository.find({
          where: { intraName: receiverIntraName },
        })
      ).length === 0
    )
  ...
  async getFriendship(accessToken: string): Promise<any> {
    return await this.getFriendshipByIntraNAme(
      (
        await this.userService.getMyUserData(accessToken)
      ).intraName,
    );
  }
  ...
  async getFriendshipByIntraNAme(intraName: string): Promise<any> {
    const RECEIVER = await this.friendshipRepository.find({
      where: { receiverIntraName: intraName },
    });
    for (let i = 0; i < RECEIVER.length; i++) {
      const USER = await this.userRepository.find({
        where: { intraName: RECEIVER[i].senderIntraName },
      });
      RECEIVER[i]['userName'] = USER[0].userName;
      RECEIVER[i]['elo'] = USER[0].elo;
      RECEIVER[i]['avatar'] = USER[0].avatar;
    }
    const SENDER = await this.friendshipRepository.find({
      where: { senderIntraName: intraName },
    });
    for (let i = 0; i < SENDER.length; i++) {
      const USER = await this.userRepository.find({
        where: { intraName: SENDER[i].receiverIntraName },
      });
  ...
  async newFriendship(
    ...
    try {
      senderIntraName = (await this.userService.getMyUserData(accessToken))
        .intraName;
    } catch {
      senderIntraName = null;
    }
    const ERROR = await this.checkJson(
      senderIntraName,
      receiverIntraName,
      status,
    );
    ...
    if (status.toUpperCase() == 'ACCEPTED')
      return {
        error:
          'Friendship status (ACCEPTED) is not supported - use PATCH method to edit an existing PENDING friendship to ACCEPTED friendship instead',
      };
    if (
      (
        await this.friendshipRepository.find({
          where: {
            senderIntraName: senderIntraName,
            receiverIntraName: receiverIntraName,
          },
        })
      ).length ||
      (
        await this.friendshipRepository.find({
          where: {
            senderIntraName: receiverIntraName,
            receiverIntraName: senderIntraName,
          },
        })
      ).length
    )
      return {
        error:
          'Friendship already exist - use PATCH method to update or DELETE method to delete this existing entry',
      };
  ...
  async updateFriendship(
    ...
    if (status.toUpperCase() == 'ACCEPTED') {
      if (RECEIVER.length === 0)
        return {
          error: 'Friendship does not exist - use POST method to create',
        };
    ...
    return {
      error:
        'Friendship status (PENDING) is not supported - use POST method to create a new PENDING friendship instead',
    };
  }
