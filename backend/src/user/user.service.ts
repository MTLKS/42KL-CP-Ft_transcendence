...
export class UserService {
  ...
  async getIntraDataByIntraName(
    ...
    let response = await fetch(
      'https://api.intra.42.fr/v2/users?filter[login]=' + intraName,
      {
        method: 'GET',
        headers: { Authorization: HEADER },
      },
    ...
    if (userData.error !== undefined) return { error: userData.error };
    if (response.status !== 200 || userData.length === 0)
      return { error: userData.error };
    response = await fetch(
      'https://api.intra.42.fr/v2/users/' + userData[0].id,
      {
        method: 'GET',
        headers: { Authorization: HEADER },
      },
  ...
  async getMyAvatar(accessToken: string, res: any): Promise<any> {
    ...
    return USER_DATA[0].avatar.startsWith('https://')
      ? res.redirect(USER_DATA[0].avatar)
      : res.sendFile(
          USER_DATA[0].avatar.substring(USER_DATA[0].avatar.indexOf('avatar/')),
          { root: '.' },
        );
  }
  ...
  async newUserInfo(
    ...
    if (EXISTING.length !== 0 && accessToken !== EXISTING[0].accessToken)
      return ERROR_DELETE(
        'Invalid username - username already exists or invalid',
      );
    if (userName.length > 16 || userName.length < 1)
      return ERROR_DELETE(
        'Invalid username - username must be 1-16 characters only',
      );
    if (
      NEW_USER[0].avatar.includes('avatar/') &&
      process.env.DOMAIN + ':' + process.env.PORT + '/user/' + file.path !==
        NEW_USER[0].avatar
    )
      FS.unlink(
        NEW_USER[0].avatar.substring(NEW_USER[0].avatar.indexOf('avatar/')),
        () => {},
      );
    NEW_USER[0].avatar =
      process.env.DOMAIN + ':' + process.env.BE_PORT + '/user/' + file.path;
