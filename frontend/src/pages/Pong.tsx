...
function Pong() {
  ...
  return (
    <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2
      ...
      <div className=' absolute top-0 left-0 rounded-2xl bg-slate-950 opacity-60 w-[100%] h-[100%]' />
      <div className=' absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-row text-[300px] opacity-50'>
      ...
      >
        <div className='absolute opacity-80 bg-white rounded-md'
          style={{ top: `${player1.position}%`, left: "1%", width: "1.3%", height: "30%", transition: "all 0.15s" }}
        />
        <div className='absolute opacity-80 bg-white rounded-md'
          style={{ top: `${player2.position}%`, right: "1%", width: "1.3%", height: "30%", transition: "all 0.15s" }}
        />
        <div className='absolute opacity-80 bg-white rounded-2xl'
          style={{ top: `${ball.offset.top}%`, left: `${ball.offset.left}%`, width: "1.3%", height: "2.31%", transition: "all 0.05s" }}
        />
      </div>
    </div>
  )
  ...
  function moveBall() {
    ...
    if (newBall.offset.top <= 0 || newBall.offset.top >= 100) {
      newBall.direction.top = - newBall.direction.top;
    }
    if (newBall.offset.left <= 0) {
    ...
    if (newBall.offset.left <= 2 && newBall.offset.top >= player1.position && newBall.offset.top <= player1.position + 30) {
      newBall.direction.left = - newBall.direction.left;
      newBall.direction.top = (newBall.offset.top - (player2.position + 15)) / 30;
    }
